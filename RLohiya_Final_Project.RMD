...
test <- cbind.data.frame(test, RecType = "Test", SalePrice = 0)
...
data_train <- train_test[train_test$RecType.Train == 1, ]
data_test <- train_test[train_test$RecType.Train == 0, ]
...
R-squared is 0.93, This means 93% variance of the sale price can be explained by predictor variables in the model. F-statistic is 66.65 with 1206 of degree of freedom and p-value is also very small.
...
```{r}
predictedData_model1 <- data_test
predictedData_model2 <- Data_test
# modelColumns <- colnames(HouseDF) testDF_model <-
# testData[,colnames(testData) %in% modelColumns]

predictedData_model1$salePrice <- predict(model1, data_test)
predictedData_model2$salePrice <- predict(model2, Data_test)

Id <- data_test$Id
# Kaggle dataset for model1
salePrice <- predictedData_model1$salePrice
